...
new $Feature('dist/features/nsi/participant/Создание, Удаление - Поставщик - ЮрЛицо, без ИНН, не РФ')
.Tag('Создание')
.Tag('Удаление')
.Tag('НСИ')
.Tag('Участники')
.Tag('Поставщик')
.Tag('Поставщик.юрлицо.без-инн.не-рф')

.Feature('Поставщик (юрлицо, без ИНН, не РФ)')

.Background('', new $Sequence()
.Given('Я логинюсь под пользователем "modifier"')

.Comment('Создаем поставщика (юрлицо, без ИНН, не РФ) а затем удаляем его')
.Comment('https://wiki.proitr.ru/pages/viewpage.action?pageId=126194108')
)
...
.When('Я заполняю сгенерированный текстовый блок "Полное наименование", "ТЕСТОВЫЙ ПОСТАВЩИК"')
