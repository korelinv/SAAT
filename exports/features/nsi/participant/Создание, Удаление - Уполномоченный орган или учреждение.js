...
new $Feature('dist/features/nsi/participant/Создание, Удаление - Уполномоченный орган или учреждение')
.Tag('Создание')
.Tag('Удаление')
.Tag('НСИ')
.Tag('Участники')
.Tag('Уполномоченный-орган-или-учреждение')

.Feature('Уполномоченный орган или учреждение')

.Background('', new $Sequence()
.Given('Я логинюсь под пользователем "modifier"')

.Comment('Создаем уполномоченный орган или учреждение с обязательными полями и потом удаляем его')
.Comment('https://wiki.proitr.ru/pages/viewpage.action?pageId=126194349')
)
.Scenario('Создание', new $Sequence()
.When('Я открываю раздел системы "НСИ"')
.And('Я выбираю в меню "Участники", "Уполномоченные органы или учреждения"')
.Then('Я ожидаю, что заголовок содержит "Список уполномоченных органов или учреждений"')
...
.Then('Я нажимаю на кнопку подтверждения')
.Then('Я ожидаю, что заголовок содержит "Создание уполномоченного органа или учреждения"')
...
.And('Я заполняю сгенерированный текстовый блок "Полное наименование", "ТЕСТОВЫЙ УПОЛНОМОЧЕНЫЙ ОРГАН"')
.And('Я заполняю сгенерированный текстовый блок "Краткое наименование", "ТЕСТОВЫЙ УПОЛНОМОЧЕНЫЙ ОРГАН"')
...
.And('Я ожидаю, что заголовок содержит "Информация о уполномоченном органе или учреждении"')

.When('Я выбираю в меню "Участники", "Уполномоченные органы или учреждения"')
.Then('Я ожидаю, что заголовок содержит "Список уполномоченных органов или учреждений"')
.Then('Я заполняю фильтр "Полное наименование" сгенерированным тегом "ТЕСТОВЫЙ УПОЛНОМОЧЕНЫЙ ОРГАН"')
.And('Я жду 5 сек.')
