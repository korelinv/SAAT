...
new $Feature('dist/features/nsi/participant/Создание, Удаление - Поставщик - ФизЛицо, не РФ')
.Tag('Создание')
.Tag('Удаление')
.Tag('НСИ')
.Tag('Участники')
.Tag('Поставщик')
.Tag('Поставщик.физлицо.не-рф')

.Feature('Поставщик (физлицо, не РФ)')

.Background('', new $Sequence()
.Given('Я логинюсь под пользователем "modifier"')

.Comment('Создаем поставщика (физлицо, не РФ) а затем удаляем его')
...
.Then('Я заполняю поле "ИНН" новым кодом ИНН(ФЛ) "9909"')
.Then('Я нажимаю на кнопку с текстом "Далее"')
.Then('Я нажимаю на кнопку с текстом "Далее"')
...
.When('Я заполняю поле "Фамилия" сгенерированным тегом "ТЕСТОВЫЙ ПОСТАВЩИК"')
.And('Я заполняю поле "Имя" текстом "ИМЯ"')
.And('Я заполняю поле "Отчество" текстом "ОТЧЕСТВО"')
.And('Я заполняю поле "Адрес регистрации" текстом "asdfasdfasdf"')
