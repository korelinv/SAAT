...
new $Feature('dist/features/nsi/participant/Создание, Удаление - Контрольный орган в сфере закупок')
.Tag('Создание')
.Tag('Удаление')
.Tag('НСИ')
.Tag('Участники')
.Tag('Контрольные-органы-в-сфере-закупок')

.Feature('Контрольный органа в сфере закупок')

.Background('', new $Sequence()
.Given('Я логинюсь под пользователем "modifier"')

.Comment('Создаем контрольный органа в сфере закупок с обязательными полями а зетем удаляем его')
.Comment('https://wiki.proitr.ru/pages/viewpage.action?pageId=126196005')
)
.Scenario('Создание', new $Sequence()
.When('Я открываю раздел системы "НСИ"')
.And('Я выбираю в меню "Участники", "Контрольные органы в сфере закупок"')
.Then('Я ожидаю, что заголовок содержит "Список контрольных органов в сфере закупок"')

.When('Я нажимаю на кнопку создания')
.Comment('And  Я заполняю визард для контрольного органа в сфере закупок')
...
.Then('Я нажимаю на кнопку подтверждения')
.Then('Я ожидаю, что заголовок содержит "Создание контрольного органа в сфере закупок"')
...
.And('Я заполняю сгенерированный текстовый блок "Полное наименование", "ТЕСТОВЫЙ КОНТРОЛЬНЫЙ ОРГАН"')
.And('Я заполняю сгенерированный текстовый блок "Краткое наименование", "ТЕСТОВЫЙ КОНТРОЛЬНЫЙ ОРГАН"')
...
.And('Я ожидаю, что заголовок содержит "Информация о контрольном органе в сфере закупок"')

.And('Я выбираю в меню "Участники", "Контрольные органы в сфере закупок"')
.Then('Я ожидаю, что заголовок содержит "Список контрольных органов в сфере закупок"')
.Then('Я заполняю фильтр "Полное наименование" сгенерированным тегом "ТЕСТОВЫЙ КОНТРОЛЬНЫЙ ОРГАН"')
.And('Я жду 5 сек.')
